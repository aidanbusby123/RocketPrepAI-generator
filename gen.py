...
def load_prompts():
    ...
    with open ("prompts/craft_and_structure/cross_text_connections.txt", "r") as f:
        cross_text_connections_prompt = f.read()

    
    # Information and Ideas
    with open ("prompts/information_and_ideas/central_ideas_and_details.txt", "r") as f:
        central_ideas_and_details_prompt = f.read()

    with open ("prompts/information_and_ideas/command_of_evidence.txt" , "r") as f:
        command_of_evidence_prompt = f.read()

    with open ("prompts/information_and_ideas/inferences.txt", "r") as f:
        inferences_prompt = f.read()


    # Standard English Conventions

    with open ("prompts/standard_english_conventions/boundaries.txt", "r") as f:
        boundaries_prompt = f.read()

    with open ("prompts/standard_english_conventions/form_structure_and_sense.txt", "r") as f:
        form_structure_and_sense_prompt = f.read()

    # Expression of Ideas

    with open ("prompts/expression_of_ideas/rhetorical_synthesis.txt", "r") as f:
        rhetorical_synthesis_prompt = f.read()

    with open("prompts/expression_of_ideas/transitions.txt", "r") as f:
...
def format_question(raw_data, domain, skill_category, difficulty):
    format_prompt = prompts["format_prompt"]
    format_prompt = format_prompt.format(domain=domain, skill_category=skill_category, difficulty=difficulty)
...
def generate_question(system_prompt, user_prompt, messages, domain, skill_category, difficulty):
    ...
    formatted_response = format_question(gemini_response, domain=domain, skill_category=skill_category, difficulty=difficulty)
